...
float step_alpha = 0.2, step_gama = 0.0, raio = 1.0, gama = 0.0, gamae = 0.0, gamad = 0.0;
...
void display() {
    ...
    objetos[1]->desenharObjeto(1, 3);
    glPopMatrix();

    glPushMatrix();
    glTranslatef(objetos[2]->posX + gamae, objetos[2]->posY, objetos[2]->posZ - gamae);
    glRotatef(-90.0, 1.0, 0.0, 0.0);
    objetos[2]->desenharObjeto(1, 3);
    glPopMatrix();

    glPushMatrix();
    glTranslatef(objetos[3]->posX - gamad, objetos[3]->posY, objetos[3]->posZ - gamad);
    glRotatef(-90.0, 1.0, 0.0, 0.0);
    objetos[3]->desenharObjeto(1, 3);
    glPopMatrix();
...
GLuint LoadTexture(const char * filename, int width, int height) {
    ...
    fgets(linha2,sizeof linha2, file);
    fgets(linha2,sizeof linha2, file);
    fgets(linha2,sizeof linha2, file);
...
int main(int argc, char **argv){
    if(argc != 4){
        cout << "\nUso: " << argv[0] << " <arquivo.obj> <arquivo.obj> <textura.ppm>" << endl;
        return 1;
    }
    glutInit(&argc, argv);
    ...
    texture[0] = LoadTexture(argv[3], 500, 333);
