...
void Objeto::desenharObjeto(int textIndex){
    ...
    for(i = 0; i < numFaces; i++) {
		if(i % 2 == 1) {
            ...
            glVertex3f(cx[faces[i].I1], cy[faces[i].I1], cz[faces[i].I1]);
            glTexCoord2f(TexCoordX+1.0/(float)hsamples, TexCoordY+(1.0/(float)vsamples));
            glVertex3f(cx[faces[i].I2], cy[faces[i].I2], cz[faces[i].I2]);
            glTexCoord2f(TexCoordX, TexCoordY);
            glVertex3f(cx[faces[i].I3], cy[faces[i].I3], cz[faces[i].I3]);
            glEnd();
		} else {
		    glBegin (GL_POLYGON);
            glNormal3f(Ni[i],Nj[i],Nk[i]);
